...
class TestFactsService:
    ...
    def test_get_last_native_name_record(self, mocker, facts_service):
        expected_url = ('https://facts.prod.ecorithm.com/api/'
                        'v1/building/26/last-native-name-record')
        mock_get = mocker.patch.object(facts_service, 'get',
                                       return_value='mock-response')
        mock__get_parser = mocker.patch.object(facts_service, '_get_parser',
                                               return_value={'parser':
        ...
        result = facts_service.get_last_native_name_record(
            building_id=26, native_name='native-name-1',
            max_time='2017-12-20 00:00')
        mock_get.assert_called_once_with(expected_url, data={
            'native_name': 'native-name-1', 'max_time': '2017-12-20 00:00'})
        mock__get_parser.assert_called_once_with('json')
        mock__format_response.assert_called_once_with('mock-response',
                                                      parser='mock-parser',
                                                      parser_args={'arg': 1})
